...
static GstStaticPadTemplate gst_rtp_raop_sink_ctrl_template =
    GST_STATIC_PAD_TEMPLATE ("sink_ctrl", GST_PAD_SINK, GST_PAD_REQUEST,
...
static GstStaticPadTemplate gst_rtp_raop_src_ctrl_template =
    GST_STATIC_PAD_TEMPLATE ("src_ctrl", GST_PAD_SRC, GST_PAD_REQUEST,
...
static void gst_rtp_raop_set_property (
    GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gst_rtp_raop_get_property (
    GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static gboolean gst_rtp_raop_src_event (
    GstPad *pad, GstObject *parent, GstEvent *event);

static gboolean gst_rtp_raop_sink_event (
    GstPad *pad, GstObject *parent, GstEvent *event);
static GstFlowReturn gst_rtp_raop_chain (
...
gst_rtp_raop_class_init (GstRtpRaopClass *klass)
{
  ...
  gst_element_class_set_details_simple (gstelement_class, "RTP ROAP Muxer",
      "Filter/Network/RTP",
  ...
  gstelement_class->release_pad = GST_DEBUG_FUNCPTR (gst_rtp_raop_release_pad);
}
...
gst_rtp_raop_init (GstRtpRaop *raop)
{
  ...
  gst_pad_set_event_function (
      priv->sinkpad, GST_DEBUG_FUNCPTR (gst_rtp_raop_sink_event));
  gst_pad_set_chain_function (
      priv->sinkpad, GST_DEBUG_FUNCPTR (gst_rtp_raop_chain));
  ...
  gst_pad_set_event_function (
      priv->srcpad, GST_DEBUG_FUNCPTR (gst_rtp_raop_src_event));
...
static gboolean
gst_rtp_raop_ctrl_sink_event (GstPad *pad, GstObject *parent, GstEvent *event)
{
  gboolean ret = TRUE;
  GstRtpRaop *raop;
    ...
    const gchar *name, const GstCaps *filter)
{
  ...
  if (template == gst_element_class_get_pad_template (klass, "sink_ctrl")) {
    ...
    pad = gst_pad_new_from_static_template (
        &gst_rtp_raop_sink_ctrl_template, "sink_ctrl");

    gst_pad_set_chain_function (pad, gst_rtp_raop_ctrl_chain);
    gst_pad_set_event_function (
        pad, (GstPadEventFunction) gst_rtp_raop_ctrl_sink_event);
    ...
    gst_element_add_pad (element, pad);
             ...
             gst_element_class_get_pad_template (klass, "src_ctrl")) {
    if (priv->ctrl_srcpad) {
      GST_WARNING_OBJECT (raop, "pad already requested");
      return NULL;
    }
    ...
    pad = gst_pad_new_from_static_template (
        &gst_rtp_raop_src_ctrl_template, "src_ctrl");
...
gst_rtp_raop_plugin_init (GstPlugin *plugin)
{
  ...
  return gst_element_register (
      plugin, "rtpraop", GST_RANK_NONE, GST_TYPE_RTP_RAOP);
}
