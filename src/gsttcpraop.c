...
static gboolean gst_tcp_raop_set_sink_caps (GstBaseParse *parse, GstCaps *caps);
static GstFlowReturn gst_tcp_raop_handle_frame (
    GstBaseParse *parse, GstBaseParseFrame *frame, gint *skipsize);
...
gst_tcp_raop_class_init (GstTcpRaopClass *klass)
{
  ...
  parse_class->set_sink_caps = GST_DEBUG_FUNCPTR (gst_tcp_raop_set_sink_caps);
  parse_class->handle_frame = GST_DEBUG_FUNCPTR (gst_tcp_raop_handle_frame);
}
...
gst_tcp_raop_set_sink_caps (GstBaseParse *parse, GstCaps *caps)
{
  ...
  if (!gst_structure_get_int (structure, "clock-rate", &priv->clock_rate)) {
    GST_WARNING_OBJECT (
        raop, "No clock rate provided: set to default %d!", DEFAULT_CLOCK_RATE);
    priv->clock_rate = DEFAULT_CLOCK_RATE;
  }
...
gst_tcp_raop_plugin_init (GstPlugin *plugin)
{
  GST_DEBUG_CATEGORY_INIT (
      gst_tcp_raop_debug, "tcpraop", 0, "RAOP RTP stream depayloader");

  return gst_element_register (
      plugin, "tcpraop", GST_RANK_NONE, GST_TYPE_TCP_RAOP);
}
